...
Students open their real class, tick the posts they want help with, and get tutored from those exact documents. Every explanation cites a passage the student can click to open. The tutor checks what the student understands and changes what it teaches next.
...
4. Ask a question that needs both documents. Click a citation to jump to the passage.
5. Click **Teach me**. Answer the diagnostic wrong. Watch the tutor reteach and highlight the teacher's own material.
6. Explain the idea back. Get a recap of what you showed and what to revisit.
7. Close the overlay. You are on the same Classroom page. Reopen to resume.
...
| API | `app/services/api` | Bun and SQLite. Verifies the student's Google access, stores lessons and evidence |
| Classroom client | `app/services/classroom` | Classroom, Drive, and Docs APIs. Reads posts and extracts passages from attachments |
| Tutor | `app/services/agent` | Strict JSON output, verified citations, enforced lesson state machine |
| Study UI | `app/apps/web` | Document tabs, lesson thread, whiteboard, optional voice |
...
cd app
bun install
cp .env.example .env   # add GOOGLE_CLIENT_ID and the model settings
bun run build
bun run start
```
...
| Path | Role |
| --- | --- |
| `app/` | Extension, study UI, API, tutor, tests |
